...
const htmlRenderer = {
  ...
  updateInlineCss(element, styles) {
  ...
  '/': {
    htmlOfBannerNavigationButton(index) {
    ...
    htmlOfUserName(username) {
      return `<span> ${username}</span>`;
    },

    htmlOfProfilePicture(profilePicture) {
    ...
    renderFlashSale(data) {
      this.renderFlashSaleClock(data.start, data.end);
      this.renderFlashSaleItems(data.items);
      htmlRenderer.updateInlineCss(".flash-sale-tiles", `--number-of-items: ${data.items.length}`);
    },

    renderFlashSaleClock(start, end) {
    ...
    htmlOfVoucherBanner(image, name) {
      return `<img src="${image}" alt="${name}">`;
    },

    htmlOfVoucherLink(link) {
      return `<a href="${link}" target="_blank" rel="noopener noreferrer"></a>`;
    },

    renderVoucherBanners(data) {
    ...
    htmlOfSearchAd(keyword, link = null, image = null) {
  ...
  '/product': {
    ...
    renderProduct(data) {
      if (data) {
        ...
        document.querySelector('.product-variations').innerHTML = this.htmlOfVariations({ catergories: data.catergories, variations: data.variations });
